...
### محاسبه با رابطه پاسکال

یک روش دیگر برای محاسبه‌ی ترکیب، استفاده از **رابطه‌ی پاسکال** است. این رابطه می‌گوید:

$$
C(n, r) = C(n-1, r-1) + C(n-1, r)
$$

ایده‌ی اصلی این است که مقادیر ترکیب را از کوچک‌ترین حالت‌ها شروع کنیم و به صورت جدولی محاسبه کنیم.

حالت‌های پایه به صورت زیر هستند:
...
بنابراین می‌توانیم یک جدول `dp` بسازیم که در آن `dp[n][r]` برابر با $C(n,r)$ باشد.
...
در هر خانه‌ی جدول، مقدار آن از دو خانه‌ی ردیف قبلی به دست می‌آید:

$$
dp[i][j] = dp[i-1][j-1] + dp[i-1][j]
$$

به همین دلیل هر خانه فقط یک بار محاسبه می‌شود.

---

### پیچیدگی

برای هر `i`، حداکثر `i` مقدار محاسبه می‌کنیم. بنابراین تعداد کل عملیات برابر با:
...
همچنین برای نگهداری جدول به $O(n^2)$ حافظه نیاز داریم.

!!! info "نکته"
اگر فقط مقدار $C(n,r)$ را برای یک `n` و `r` مشخص می‌خواهیم، می‌توانیم با نگه‌داشتن فقط دو ردیف آخر، حافظه را به $O(n)$ کاهش دهیم؛ اما زمان همچنان $O(n^2)$ باقی می‌ماند.
